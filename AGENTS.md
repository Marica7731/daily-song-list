...
- Each build uses one fixed **32,000,000,000-byte** sparse ext4 loop filesystem below its exact owner-marked `/var/tmp/dsl-wdc-volume-<run>-<attempt>` root. `/var/tmp` must be a real directory on the same host filesystem as the project. The checker counts both project logical bytes and allocated host blocks without following links, while the volume remains outside the project tree so its sparse logical length cannot inflate project accounting.
