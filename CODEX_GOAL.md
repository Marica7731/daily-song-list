...
- 当前状态：`check-gate-followup-tested`。受限 WDC 服务端构建已由 PR `#86` 合并；latest accepted 为 `accepted_32561332302_1` 且在线 PG 身份门禁通过。Check code `32561363075` 暴露的 README 误路由已在本分支以 fail-closed 章节分类器修复，本地 next-serving `205/205`、relay `13/13`、3 个 workflow YAML 与 22 个 shell block、Python/shell 语法及 diff check 全部通过。WDC 仍为旧 release，交付未完成。
...
- 下一步：提交并合并 README Check gate follow-up；确认新 Check code 精确跳过陈旧根 Node 套件后，只运行一个 latest-head WDC，并做公网 API/浏览器验收、10 分钟观察与最终精确清理。
