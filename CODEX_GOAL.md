...
- `in_progress`：run `32473191786` 暴露 authoritative 7d Artist 列表仍按 raw title 而非 accepted `songKey` 聚合；已完成精确只读生产核验，并在隔离 worktree实现 canonical-key 聚合、真实形态回归与首个 ranking combo 前的全 revision 门禁；测试、PR/CI/squash merge完成前禁止重跑。
