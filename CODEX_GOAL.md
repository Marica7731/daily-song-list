...
- 当前状态：`wdc-song-reset-ranking-owner-fix`。PR `#122` 已修复 Ubuntu gate sparse checkout，latest main=`ffba02a190007bfc329136726fc163adc2d8abc8`。唯一 WDC `32701655614` / sync job `97354594116` 在完成受限 build 的 24 个基础 view combo 后，于 `affected-parent-sources` cardinality gate 精确失败：`all/songs/0007036316d9dffa` ranking=`770/1/736/770`、source=`771/1/737/771`。生产只读证据把缺失项锁定为视频 `PZPwqBtYM2I`、seconds=`14304` 的 accepted full-reset 候选；source 侧已按 persisted `parentSongGroupKey` 正确回绑，ranking 侧却按 raw artist spelling 建了另一个组。当前隔离 worktree 正在把 ranking 候选仅按 persisted source-authority 的 `video+seconds+title+artist` 唯一证据回绑；歧义或 owner 不在有界父窗口时 fail closed。失败 run 的 32GB 临时卷、build unit 和 VPS2 relay 均已清理，WDC 未 bundle/deploy，线上仍为旧 release，交付未完成。
...
- 下一步：完成 ranking/source owner 对称修复的精确 `771/1/737` 回归与全量 next-serving/relay/syntax/diff 测试，commit/push/PR/CI/squash merge；重新确认 latest main、latest accepted、零并发 writer、资源门禁后只运行一个 latest-head WDC，并按 checkpoint 续接完成公网 API/浏览器验收、10 分钟观察与最终精确清理。
