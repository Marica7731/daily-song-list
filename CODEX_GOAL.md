...
- 当前状态：`wdc-ubuntu-gate-sparse-fix`。受限 code-only API deploy run `32700659060` / job `97351318366` 已在 main `d6de671875d313cb2aaabef03798f5ecd9499462` 成功：未写 canonical PostgreSQL，绑定 active=`accepted_32589432063_1`、content=`86fbd44bd80a4e21429f384d8216077facc4c80583ac68cd81f01e4470af1857`、source=`7c7f7c339c40fb0d50d1d2b7a40fb82c55e3dcc2`；identity endpoint 实测约 `0.218s`，非法参数 HTTP 400，完整 health HTTP 200。随后唯一 WDC `32701028142` 在 Ubuntu gate 因 sparse checkout 漏掉测试直接导入的 `server/pg_api_server.py` 而快速失败；sync writer job skipped，WDC/VPS2 无临时卷、relay 或发布写入。当前同轮补齐 gate checkout、AST 和精确回归；线上仍为旧 release，交付未完成。
...
- 下一步：完成 WDC Ubuntu gate sparse-input 修复的全量测试、PR/CI/squash merge；确认零并发 writer 后只运行一个 latest-head WDC，并按 checkpoint 续接完成公网 API/浏览器验收、10 分钟观察与最终精确清理。
