...
- 2026-08-24 21:32 Asia/Taipei（当前权威状态）：唯一 latest-head WDC `32721212906`（head=`fcda454bc40c74f15c5a49faff4784b2d6d96a2d`，Ubuntu job `97412843779` success，sync job `97413297133` failure）已完成 24/24 view combo、VTuber affected source `675/675` 与 parent source `661/661` 后，在 `all/songs/068ca49181ea5506` 精确失败：ranking=`50/1/49/50`、source=`51/1/50/51`。只读生产重放证明 persisted owner=`キミシダイ列車::oneokrock` 的父 source 为 `50/49`；reset candidate `6aqknyaOJWY:8:2709` 有 exact parent owner，普通 overlay `z0WiDRGbEo8:214:2232` 只有同 raw `キミシダイ列車::one ok rock`，ranking 按既有 mixed-group 合同保留独立 `1/1` raw 卡，但 source 的 broad canonical fallback 错把该 overlay 再吞回 persisted owner。当前修复让 candidate source 路由复用 exact reset owner 证据，并在 mixed raw group 中排除普通 overlay；raw payload、显式 runtime curation 与 fail-closed ambiguity 均不变。受影响 Song source 的最终 exporter/cardinality checkpoint 已前移到 VTuber 与通用 source 大阶段之前，且后续阶段扣除完成 key、不重复复制。补丁生产只读重放结果为主 ranking/source 都 `50/1/49/50`，raw 卡仍 `1/1/1/1`；focused `8/8`、next-serving `263/263`、relay/资源 `13/13`、two-day `4 passed + 2 skipped` 均通过。失败 run 的 32GB volume/build/relay 已 cleanup，cgroup peak约 `1.1GB`、swap `0`，生产 release 未写入；待 commit/push/PR/CI/squash merge 后才允许唯一新 WDC。
...
- 当前下一步（取代上方旧下一步）：提交 `068ca49181ea5506` mixed raw source 路由修复与早期 Song cardinality preflight，push/PR/CI/squash merge；重新读取 latest main/accepted，确认无重复 WDC、无并发 writer、VPS2 relay inactive 与全部 32GB/16GB/20GB/2.5GB/1GB/16GB 资源门禁后，只调度一次 latest-head WDC。成功后必须完成三个历史目标 source/ranking、health/meta/release、四类 Artist 视图、31-video 跨页、多 occurrence、搜索/双筛选、真实浏览器、同协议延迟和连续 10 分钟观察，再做 exact run-owned cleanup。
