...
- 2026-08-25 03:32 Asia/Taipei（当前权威状态，取代下方旧状态）：唯一 latest-head WDC `32757942466`（head=`3901f39138c73c7e3079bafbaee0f6c10f412685`，Ubuntu job `97529722325` success，sync job `97530291539` failure）完成 `24/24` views、7d authoritative sources `4500/4500`、全量 Artist/Song/affected owner 门禁，并推进 affected-parent checkpoint `500/8519` 后，在 `all/songs/10df4dffdbdef345` 精确 fail closed：ranking=`189/1/188/189`、source=`190/1/189/190`。生产只读核验确认 persisted card=`君と夏フェス::shishamo`，基线 `188/187`，4 条 parent reset 后有 5 条 exact `SHISHAMO` candidate；另有同 title 但 raw artist=`＠SHISHAMO` 的独立 occurrence `dUmeM96Zy-Q:221:3221`。ranking 正确保留后者为 raw `君と夏フェス::@shishamo` 卡，source 的 punctuation-insensitive broad owner 却错误吸入 persisted card。当前最小修复仅在 ordinary candidate 同时匹配 persisted display owner 时允许 broad owner fallback；raw source key、exact accepted-reset owner、runtime change、payload 与 cardinality fail-closed 均不改。生产 `REPEATABLE READ READ ONLY` 重放得到 persisted=`189/1/188`、raw `＠SHISHAMO=1/1/1` 且 persisted source 不含 `dUmeM96Zy-Q`；完整 next-serving `267/267`、relay `13/13`、Python compile 与 `git diff --check` 通过。失败 run cleanup success，固定卷 cap=`31,317,221,376` bytes、末次 used=`3,294,027,776` bytes，MemoryPeak约 `990.3MB`、swap=`0`，WDC 项目 `7,611,437,438` bytes、cleanup 后宿主可用 `79,273,234,432` bytes，当前 release 仍为 `3cfb9f8b327534d8f52ec397f2527d849f879708b105d0857de8e5b92d977d85`；交付未完成。
- 2026-08-25 01:30 Asia/Taipei（当前权威状态，取代下方旧状态）：唯一 latest-head WDC `32744558790`（head=`277f8c7ca017d40485a2c420aa5976fc35018932`，Ubuntu job `97486867364` success，sync job `97487512656` failure）完成 `24/24` views、7d authoritative sources `4500/4500`、全量 Artist/Song/affected owner 门禁后，在 affected-parent Song source 精确 fail closed：`all/songs/00f3ae807208eb9b` ranking=`5/1/5/5`、source=`52/1/52/52`。生产只读回放确认该 raw source 为 `晩餐歌::tuki.`，其中 4 条 exact accepted-reset tuple 的 persisted owner 实为 `晩餐歌::tuki`（source `1f302657d7e35049`），其余 5 条才属于 raw source。根因有两层：synthetic Song source 缺 persisted detail type 时错误启用兼容 7d reset；source 路径又未复用 ranking 的 exact accepted-reset persisted-owner 证据。当前最小修复按 synthetic source 的精确 target view 选择 Song range contract，并只把 exact reset-owned candidate 路由到 persisted owner；普通 raw tuple、raw payload、canonical PostgreSQL 和 cardinality fail-closed 均不改。精确生产只读重放已由 `9/9` 收敛为 `5/5`，完整 next-serving `266/266`、relay/storage/release `13/13`、Python compile 与 `git diff --check` 通过。失败 run cleanup success，build/root/volume/relay 均 absent，MemoryPeak约 `1.1GB`、swap=`0`，WDC 项目 `7,611,437,438` bytes、宿主可用 `79,281,037,312` bytes，当前 release 仍为 `3cfb9f8b327534d8f52ec397f2527d849f879708b105d0857de8e5b92d977d85`；交付未完成。
- 2026-08-24 23:10 Asia/Taipei（当前权威状态）：PR `#125` 的 mixed reset 修复已合并为 main=`0319be275075a0fc0a26b35786a449a1745d3c2e`；随后唯一 latest-head WDC `32734330269`（Ubuntu job `97453489930` success，sync job `97454040968` failure）完成 24/24 view combo、7d authoritative sources `4500/4500` 与全部 Artist/Song/affected owner 门禁后，在首段 affected-parent Song source 精确失败：`all/songs/09e7b9fb658dd82c` ranking=`25/1/25/25`、source=`26/1/26/26`。只读生产核验确认 persisted owner=`焔::unknown` 的 full-runtime source 为 `25/1/25`；普通 all-range overlay `accepted_30884784837_1 / cDd1kQ62M5M:206:3260` 的 raw artist=`未記載`，ranking 保留独立 raw `焔::未記載` 卡，但 source candidate 路由错误复用了 old-side unknown-artist 归一化并吸入 persisted owner。当前最小修复只让 ordinary candidate source 使用与 ranking 相同的 raw artist/canonical fallback；exact reset owner、显式 runtime curation、payload 与 ambiguity fail-closed 均不变。精确生产只读重放已得到 persisted=`25/1/25`、raw=`1/1/1`，focused `5/5`、完整 next-serving `264/264`、relay/资源 `13/13` 通过。失败 run cleanup success；固定卷总量 `31,317,221,376` bytes，cgroup MemoryPeak=`737,193,984` bytes、swap peak=`0`，WDC 项目恢复 `7,611,437,438` bytes、宿主可用 `79,285,964,800` bytes，旧 release 未改变。待 commit/push/PR/CI/squash merge 后，重新确认 latest main/accepted 与零并发 writer，再只运行一个 latest-head WDC。
- 当前状态：`wdc-song-reset-ranking-occurrence-owner-fix`。PR `#123` 已修复 `0007036316d9dffa` 的单条 reset owner 并 squash merge 为 latest main=`a7879ba457f39c1d8f090197c6199c3b20e565ba`。唯一 WDC `32709675902` / sync job `97378645220` 完成 24/24 view combo、7d authoritative sources `4500/4500`、affected-parent sources `675/675` 与 parent sources `661/661` 后，在 `all/songs/00f9f259ea664bf2` 精确失败：ranking=`1704/1/1677/1704`、source=`1700/1/1673/1700`。生产只读复现确认 persisted owner=`夜明けと蛍::nbuna`：full-runtime 基线 `1702/1675`，reset 删除 `5/5`，同一 raw `n-buna` candidate 组有 `7/7`，但只有 `3/3` 具备 exact `video+seconds+title+artist` parent-source 证据；旧逻辑因三条命中而把整组七条回绑，正好多计 `4/4`。当前隔离 worktree 改为逐 occurrence 绑定 exact owner，并把同 raw 组的四条 overlay-only tuple 保持为独立 raw group；同一 exact tuple 多 owner 或 persisted owner 缺失继续 fail closed。失败 run cleanup success，32GB 临时卷/build/relay absent，WDC 未 bundle/deploy，线上仍为旧 release，交付未完成。
...
- 下一步：完成 exact `3 owned + 4 overlay-only` 分裂回归，要求 `00f9f259ea664bf2` ranking/source 同为 `1700/1/1673/1700`，并重跑既有 `0007036316d9dffa=771/1/737` 回归、全量 next-serving/relay/syntax/diff；commit/push/PR/CI/squash merge后，重新确认 latest main/accepted、零并发 writer和全部资源门禁，只运行一个 latest-head WDC，再完成公网 API/浏览器验收、10 分钟观察与精确清理。

- 当前下一步（取代上方旧下一步）：提交 `09e7b9fb658dd82c` unknown-placeholder candidate/source 边界修复，push/PR/CI/squash merge；重新读取 latest main/accepted，确认无重复 WDC、无并发 writer、VPS2 relay inactive 与全部 32GB/16GB/20GB/2.5GB/1GB/16GB 资源门禁后，只调度一次 latest-head WDC。成功后必须完成四个历史目标 source/ranking、health/meta/release、四类 Artist 视图、31-video 跨页、多 occurrence、搜索/双筛选、真实浏览器、同协议延迟和连续 10 分钟观察，再做 exact run-owned cleanup。
- 当前下一步（2026-08-25 01:30，取代上方旧下一步）：仅提交 synthetic Song range contract 与 exact accepted-reset owner 路由修复，push/PR/CI/squash merge；随后重新读取 latest main/accepted、实际 writer/relay 与资源门禁，确认零并发 writer、零实际重复 WDC后只调度一次 latest-head WDC。新 run 必须先证明 `00f3ae807208eb9b=5/1/5/5`，并继续通过全部历史 target、bundle/deploy、公网 API/浏览器、10 分钟观察与 exact cleanup；失败只依据 checkpoint/完整异常继续最小修复，禁止盲重跑。
- 当前下一步（2026-08-25 03:32，取代上方旧下一步）：仅提交 persisted-display-bounded ordinary Song candidate 路由与 `＠SHISHAMO` 精确回归，push/PR/CI/squash merge；重新读取 latest main/accepted、actual writers、relay 与全部固定资源门禁，确认零并发 writer、零实际重复 WDC后只调度一次 latest-head WDC。新 run 必须通过 `10df4dffdbdef345=189/1/188/189` 与全部历史 target，随后完成 bundle/deploy、公网 API/浏览器、10 分钟观察和 exact cleanup；任何失败只从完整 checkpoint/异常做最小修复，禁止盲重跑。
- 2026-08-25 04:19 Asia/Taipei（当前权威状态，取代上方旧下一步）：唯一 latest-head WDC `32804165453`（head=`be06dd412dcd1645560b4054441c7805c5d73048`，Ubuntu job `97670841950` success，sync job `97671209264` failure）完成 7d `12/12`、all views 已开始、7d authoritative sources `4500/4500`、Artist/Song/affected 全量 owner/preflight 与 source scope `3434 videos/60053 sources/123601 pairs` 后，在 affected-parent Song source `00e65040921c60a9` 精确 fail closed：ranking=`283/1/283/283`、source=`282/1/282/282`。生产只读核验确认 persisted full-runtime owner=`ヒロイン::backnumber`，source parent=`282/1/282/282`；accepted 7d boundary row `b9cPXiElWWk:125:1753`（raw artist=`back number`、sourceSystem=`core-7d`）已进入 all ranking，却被 source candidate range split 排除。不是 transport/OOM/身份或资源问题。当前最小修复仅将 manifest-validated newest 7d boundary rows投影进 Song all-range source inputs，并加 provenance marker；仅该 marker允许 persisted display-owner 的空格差异，ordinary unranked alias 仍受 raw-group 门禁隔离。新增 source-input projection 与 `ヒロイン::backnumber` `2/1/2/2` 回归，同时保留 `6月/六月`、`＠SHISHAMO` 等负例；当前 focused `8/8` 已通过，尚未 commit/PR/WDC 重跑。失败 cleanup success：build/root/relay absent，WDC 未 bundle/deploy，资源未越界；线上仍是旧 release `3cfb9f8b327534d8f52ec397f2527d849f879708b105d0857de8e5b92d977d85`，交付未完成。
- 2026-08-25 14:02 Asia/Taipei（当前权威状态）：WDC `32811657387`（head=`ea91dc123194f836bbba5fddb2d77ef293853e87`，Ubuntu job=`97691979662` success，sync job=`97692324899` failure）在 `all/songs/00e65040921c60a9` affected-parent cardinality gate 再现 `ranking=283/1/283/283`、`source=282/1/282/282`。7d sources `4500/4500`、all scope `3434 videos/60053 sources/123601 pairs` 与 Artist/Song/affected owner preflight 均已通过；资源/transport 正常（MemoryPeak约 `993.6MB`、swap=`0`、volume末次约 `3.10GB/31.317GB`、项目约 `7.63GB`、宿主可用约 `76.1GB`），失败后 run-owned cleanup success。生产只读 lineage 显示 active accepted=`accepted_32589432063_1` 的 persisted source detail=`282`；affected-parent source plan 使用 narrowed detail lineage，未把完整 active overlay lineage 传给 Song source 的 authoritative 7d lookup，因此 ranking 已含 boundary occurrence 而 source 少 1。最小修复为 `_snapshot_source_overlay_inputs(... authoritative_7d_revision_ids=...)`，仅 Song all-range ordinary source lookup 使用 materializer 的完整 active lineage；普通 overlay/reset/runtime contract 不变。新增 narrow-plan/full-lineage 回归；focused `3/3`、完整 next-serving/relay/two-day `290` tests（skipped=`2`）、Python compile 与 diff check 均通过；尚未 commit/PR/WDC 重跑，交付未完成。
- 当前下一步（2026-08-25 14:02）：提交本轮完整-lineage Song source 修复，push/PR/CI/squash merge；重新读取 latest main/accepted、实际 writer/relay 与固定卷/cgroup/宿主/relay 门禁，确认零并发 writer、零实际重复 WDC 后只调度一次 latest-head WDC。新 run 必须通过 `00e65040921c60a9=283/1/283/283`、`10df4dffdbdef345=189/1/188/189`、`0007036316d9dffa=771/1/737` 与全部历史 target，随后完成 bundle/deploy、公网 API/浏览器、10 分钟观察和 exact cleanup；任何失败继续从完整 checkpoint/异常做最小修复，禁止盲重跑。
...
| --- | --- | --- | --- | --- |
| `32757942466` | `3901f39138c73c7e3079bafbaee0f6c10f412685` | server-side `affected-parent-sources` cardinality gate；`24/24` views、7d authoritative `4500/4500`、全量 owner/preflight 与 checkpoint `500/8519` 已完成 | `all/songs/10df4dffdbdef345` ranking=`189/1/188/189`、source=`190/1/189/190`。persisted owner=`君と夏フェス::shishamo`；ordinary overlay `dUmeM96Zy-Q:221:3221` 的 raw artist=`＠SHISHAMO`，ranking 保留独立 raw card，而 source broad owner 去标点后把它吸入 persisted card | broad owner fallback 仅在 candidate 同时匹配 persisted display owner 时有效；raw source key 与 exact reset owner 不变。生产 read-only 重放 persisted=`189/1/188`、raw=`1/1/1` 且无 `dUmeM96Zy-Q` 泄漏；next-serving `267/267`、relay `13/13`、compile/diff 通过。run cleanup success，MemoryPeak约 `990.3MB`/swap `0`，无 bundle/deploy/生产写入。 |
| `32744558790` | `277f8c7ca017d40485a2c420aa5976fc35018932` | server-side `affected-parent-sources` cardinality gate；`24/24` views、7d authoritative `4500/4500` 与全量 owner/preflight 已完成 | `all/songs/00f3ae807208eb9b` ranking=`5/1/5/5`、source=`52/1/52/52`。raw owner=`晩餐歌::tuki.`；9 条当前 raw candidate 中 4 条 exact accepted-reset tuple 属于 persisted owner=`晩餐歌::tuki` / source `1f302657d7e35049`。synthetic Song source 缺 detail type 时误开兼容 7d reset，且 source 未复用 ranking 的 exact reset owner 证据 | synthetic source 按 exact target view 解析 Song contract；exact reset candidate 继承已 fail-closed 的 persisted owner source，普通 tuple 仍留 raw card。生产只读重放 raw source=`5/5`；next-serving `266/266`、relay/storage/release `13/13`、compile/diff 通过。run cleanup success，peak约 `1.1GB`/swap `0`，无 bundle/deploy/生产写入。 |
| `32734330269` | `0319be275075a0fc0a26b35786a449a1745d3c2e` | server-side 首段 `affected-parent-sources` cardinality gate；24/24 view combo、7d authoritative `4500/4500`、全量 owner/preflight 已完成 | `all/songs/09e7b9fb658dd82c` ranking=`25/1/25/25`、source=`26/1/26/26`。persisted owner=`焔::unknown`；普通 overlay `accepted_30884784837_1 / cDd1kQ62M5M:206:3260` 的 raw artist=`未記載`。ranking 保留 raw `焔::未記載` 卡，source candidate 路由却用 unknown flag 将它吸入 persisted owner | ordinary candidate source 改为镜像 ranking 的 raw artist/canonical fallback；exact reset owner 与 runtime old-side 权威逻辑不变。生产 REPEATABLE READ READ ONLY 重放：persisted=`25/1/25`、raw=`1/1/1`；focused `5/5`、next-serving `264/264`、relay `13/13`。run cleanup success，MemoryPeak=`737,193,984`、swap=`0`、未 bundle/deploy、生产未写入。 |
| `32721212906` | `fcda454bc40c74f15c5a49faff4784b2d6d96a2d` | server-side `affected-parent-sources` cardinality gate；24/24 view combo、VTuber affected `675/675` 与 parent `661/661` 已完成 | `all/songs/068ca49181ea5506` ranking=`50/1/49/50`、source=`51/1/50/51`。reset-owned `6aqknyaOJWY:8:2709` 与普通 overlay `z0WiDRGbEo8:214:2232` 共用 raw `キミシダイ列車::one ok rock`；ranking 将后者保留在独立 raw 卡，source 的 broad canonical fallback 却将后者吞入 persisted `キミシダイ列車::oneokrock` | candidate source 对 mixed raw group 只接受 exact reset-owned tuple，普通 overlay 保持独立 source；新增精确 source split 回归与生产只读重放，主 ranking/source 均 `50/1/49/50`、raw 卡 `1/1/1/1`。受影响 Song source cardinality exporter 前移至 VTuber/source 大阶段前并由集成回归锁定顺序。next-serving `263/263`、relay `13/13`、two-day `4 passed + 2 skipped`；run cleanup success、cgroup peak约 `1.1GB`/swap `0`、无 bundle/deploy、生产未写入。 |
...
| `32701655614` | `ffba02a190007bfc329136726fc163adc2d8abc8` | server-side `affected-parent-sources` cardinality gate；24 个基础 view combo 已完成，source checkpoint `675/675` | `all/songs/0007036316d9dffa` ranking=`770 occurrences/1 song/736 videos/770 timestamps`、source=`771/1/737/771`。生产只读核验：父 source 与原目标均为 `771/737`；accepted full reset 的 `PZPwqBtYM2I:13:14304` raw artist=`未来古代楽団feat. 安次嶺希和子さん`，持久化 source authority owner=`忘れじの言の葉::未来古代楽団feat安次嶺希和子`。ranking reset 先减 canonical 组，再把 candidate 按 raw spelling 加到新组，导致 canonical 少 `1/1` | ranking candidate delta 复用完整 persisted source-authority，仅在 `video+seconds+raw/canonical title+artist` 唯一匹配时回绑 `parentSongGroupKey`；多 owner、缺 owner 或 owner 不在有界父窗口均 fail closed。新增同一生产形态回归，断言 ranking 与 source 都保持 `771/1/737` 且不生成 raw alias 卡；失败 run cleanup success，临时卷/build/relay absent，生产未写入 |
| `32709675902` | `a7879ba457f39c1d8f090197c6199c3b20e565ba` | server-side `affected-parent-sources` cardinality gate；24/24 view combo、7d source `4500/4500`、affected-parent `675/675`、parent `661/661` 已完成 | `all/songs/00f9f259ea664bf2` ranking=`1704/1/1677/1704`、source=`1700/1/1673/1700`。只读 trace：owner `夜明けと蛍::nbuna` 基线 `1702/1675`，reset 删除 `5/5`；raw `夜明けと蛍::n-buna` candidate 组 `7/7` 中只有 `3/3` exact 匹配 persisted source authority，旧 group-level map 却把七条全部回绑，正好多 `4/4` | 改为 candidate occurrence identity 到 owner 的逐条映射；exact 三条并回 owner，四条 overlay-only 留在 raw group并禁止 broad display fallback 重新吞并。新增 `3 owned + 4 passthrough` 回归，目标 owner=`1700/1/1673/1700`。失败 run cleanup success，cgroup peak约 `1.1GB`/swap `0`、32GB卷峰值 `6,086,991,872` bytes、无 bundle/deploy、生产未写入。 |
...
### 2026-08-24 10:00 Asia/Taipei — affected-parent Song 批处理不得共享兼容 7d reset 输入

- unique latest-head WDC run `32676865193`（head `02f734a3bf1ba737ac1b9b7ab28fbfafc90f7b2f`，sync job `97286751354`）已完成 7d/all views、7d authoritative Artist sources 及 Artist/Song/VTuber/affected-source 前置门禁，在 `affected-parent-sources` 首批 checkpoint 期间 fail closed：`all/songs/0682f35a270f7de2 ranking=(17,1,17,17) source=(18,1,18,18)`。未 bundle/deploy，生产 release 未写入；Actions exact cleanup success。
- 只读 PostgreSQL 证据保持同一 active revision `accepted_32589432063_1` / content `86fbd44bd80a4e21429f384d8216077facc4c80583ac68cd81f01e4470af1857`：Song source 的 `melancholic::unknown` all authority 为 17 条；唯一 7d overlay 是 `accepted_30347149376_1` / video `stxKMausiFw`。ranking 的 physical-range contract 保持 17，但 affected-parent source 批处理把该 7d-only row 误投影进 all，故 source 多 1 条；canonical PostgreSQL、raw payload 未修改。
- 根因是上一修复只覆盖 `server/pg_adapter.py` 的 `source_payload` generic 路径；`scripts/migration/materialize-pg-release-snapshot.py` 的 affected-parent 批处理直接以整组 source plan 调 `_snapshot_source_overlay_inputs(... include_compatible_full_reset_7d=True)`，绕过了 Song guard。
- 最小修复将该批处理的 overlay input cache 按 `(plan, include_compatible_full_reset_7d)` 分区：Song source (`type=song`) 只用 `False`，Artist/Video/unknown 继续用 `True`；写 source 时按同一 detail type 取对应 bounded input。新增回归断言 song-only affected batches 传入 `[False, False]`，不修改 raw payload、canonical PostgreSQL 或 cardinality gate。
- 本地 targeted regression `1/1`、persisted-source 相关回归 `3/3` 通过；完整本地 `254` tests 仍只有既有 Windows `subprocess.run(text=True)` GBK 解码失败 `3` 项（Linux-only installer 3 项 skipped），本轮修改相关测试均通过。失败 run 资源：volume guard 最后约 `6,335,733,760` bytes，宿主可用最低约 `72,902,676,480` bytes，project allocated `7,631,712,256` bytes，systemd memory peak 约 `1.2GB`、swap `0`；relay wire 本轮精确值未从失败日志取得，记为 `unknown`（合同上限 `16GB/2`）。cleanup 后 WDC project `7,611,437,438` bytes、宿主可用约 `79,320,412,160` bytes、root/volume/relay 均 absent/inactive。
- 交付仍未完成：本修复尚未 commit/push/PR/CI/squash merge，未调度新的 WDC；下一步必须通过全量门禁并让新 run 的 `0682...` source 与 ranking 同为 `17/1/17/17`，否则只依据完整 checkpoint/错误证据继续最小修复，禁止盲重跑。

### 2026-08-24 12:20 Asia/Taipei — mixed-range Song reset 必须保留 all 父 owner 并重绑 exact-all replacement

- unique latest-head WDC run `32682535860`（head `21fa51e5896defa7c428d48092564413b8608030`，sync job `97301834380`）完成 `24/24` 个 7d/all views、7d authoritative sources `4500/4500`、全量 source scope `3434 videos / 63942 sources / 130517 pairs`、Artist/Song/affected 前置门禁与 VTuber subset/persisted `675/675 + 661/661`；随后在首个 generic affected-parent Song source fail closed：`all/songs/0007036316d9dffa ranking=(771,1,737,771) source=(764,1,730,764)`。未 bundle/deploy，生产 release 未写入。
- 只读 PostgreSQL 精确回放确认父 ranking/source 均为 `771 occurrences / 1 song / 737 videos`，权威 owner 为 `忘れじの言の葉::未来古代楽団feat安次嶺希和子`。当前 selected reset 与该 owner 相交 7 个视频：6 个是 physical `7d` reset，1 个 `PZPwqBtYM2I` 是 exact `all` reset；`771-7=764`、`737-7=730` 与失败值完全闭合。前一修复虽然禁止 7d candidate 投影到 all，却仍把 7d reset 边界应用到 all source，错误删除 6 条不可变父 tuple；exact-all candidate 的 raw `songKey/title/artist` 又与 canonical owner 拼写不同，删除父 tuple 后没有按唯一父 source preimage 重绑，额外丢 1 条。
- 最小修复把 `include_compatible_full_reset_7d=False` 的 reset 边界收紧为“exact-range reset 或该 range 实际有 candidate 的视频”，因此 physical 7d reset 不再擦除 all 父 source，而 exact-all tombstone/reset 仍生效。对 accepted-reset Song candidate 仅允许用唯一 persisted parent source tuple `(video, seconds, raw title, raw artist)`（含 canonical-title 等价）继承当前权威 owner；多个匹配立即以 `accepted reset Song source owner is ambiguous` fail closed。raw payload、canonical PostgreSQL、ranking 与 cardinality gate 均不修改。
- 新增三项精确回归：7d reset 不作用于 all、真实 mixed reset 矩阵保持 `771/1/737` 且 `PZPwqBtYM2I` 被重绑、重复父 tuple owner 歧义 fail closed。验证结果：新增 `3/3`、全部 Song source 相关 `5/5`、完整 next-serving `257/257`、relay/storage/release contract `13/13`、Python compile 与 `git diff --check` 均通过。
- 失败 run 的最后资源证据：固定卷 total `31,317,221,376` bytes、used `6,194,126,848`，宿主可用 `73,035,321,344` bytes，project allocated/logical `7,631,716,352 / 7,615,723,212` bytes，systemd CPU `21m0.724s`、memory peak约 `1.2GB`、swap `0`；Actions 输出 `WDC_BOUNDED_CLEANUP_OK` 与 `VPS2_BOUNDED_RELAY_CLEAN`，VPS2 relay inactive。relay wire 的最终精确值仍需从完整 job log补取，未知前不得猜测。
- 当前交付未完成：本修复尚待精确清理本轮 pycache、commit/push/PR/CI/squash merge与 main Check；只有实时证明无并发 writer、无重复 WDC、latest accepted/head 身份一致且所有硬门禁齐全后，才允许只调度一个新 latest-head WDC。

### 2026-08-24 14:25 Asia/Taipei — Song ranking 不得把 physical 7d full-reset 投影进 all

- unique latest-head WDC run `32690175992`（head `538119f32e8b24b966bb6c1191a6d581644a797b`，sync job `97322626661`）完成 `24/24` views、全量 Artist/Song/affected owner preflight 与 VTuber affected/parent `675/675 + 661/661`，随后在 affected-parent source fail closed：`all/songs/0022321d6b79fa90 ranking=(11,1,11,11) source=(12,1,12,12)`。未 bundle/deploy；Actions exact cleanup success，生产 release 未写入。
- bounded 只读 PostgreSQL 诊断确认该 owner 为 `ねえ::松浦亜弥`：immutable full-runtime ranking/source 都是 `12 occurrences / 1 song / 12 videos`，compact parent occurrences 仅保留 10 条；12 个视频中只有 `iXK_smpZ5eU` 在 overlay lineage 有 physical `7d` full reset，且其 43 条 7d occurrence 不含父 all tuple `ね~え?@8056`。ranking 仍调用 compatible reset projection，错误删除该 all 父 tuple并投影 7d 集合，得到 11；Song source 已按既有同物理 range 合同正确保持 12。canonical PostgreSQL 与 raw payload 未修改。
...
- PR #118 首次 CI run `32698342840` 在导入测试时立即失败：workflow sparse checkout 未包含本次新增回归要加载的 `server/pg_api_server.py`，精确错误为 `FileNotFoundError`，不是测试断言或生产数据失败。已把该文件同时加入 PR path trigger、sparse checkout 和 Python compile 清单，并用合同断言锁定三处；不得重跑原失败配置。
- PR #118 修正后的 CI `32698450127` success 并 squash merge main `54eafff0d324affed75b026eaf2397eeb7a34893`。随后 main Check code `32698566006` 错误执行根 Node 全套并得到 `720 pass / 84 fail / 2 skipped`；精确原因是 `server/pg_api_server.py` 尚未列入 Check code 的 WDC/Python-only exclusion，sparse source 缺少无关 Node fixtures。最小流程修复仅把该文件加入 exclusion 并扩展既有精确清单回归；这些 84 项记为流程误分类，不是 accepted 数据或 WDC 失败。

### 2026-08-24 15:05 Asia/Taipei — identity-only API 必须先经 data-inert code-only 入口部署

- PR #119 已将 Check code 分类修复 squash merge 为 latest main `944947a760473f523927eb0b68bb7cc5a19010bf`；main Check `32698967842` success，并明确输出 `CODEX_NODE_TESTS_SKIPPED reason=no-node-input-changes`。此时仍未调度 WDC，因为 VPS2 运行中的旧 API 不认识 `identityOnly=1`，直接启动会在 source triplet 门禁失败。
- 现有 `deploy-pg-incremental.yml` 只会在 accepted candidate 激活路径安装 API 代码，强行复用会导入/激活数据，不符合本轮“canonical PostgreSQL 不写入”边界。新增显式 `code_only` workflow_dispatch 分支：运行于 Ubuntu hosted runner、复用 PostgreSQL release concurrency、发现任何 active/queued WDC 即 fail closed，只下载当前 `GITHUB_SHA` 的 `pg_adapter.py`/`pg_api_server.py`，不执行 `psql`、import、candidate 或 activation。
- code-only 部署必须由调用者提供 active revision/content/source 三元组；安装前用旧完整 `/api/meta` 复核，备份四个既有 Python 副本，安装后校验文件 SHA、重启 API，并要求 `/api/meta?identityOnly=1` 在 10 秒硬截止内返回同一三元组、非法值返回 400、`/healthz` 为 ok。任一异常恢复旧文件、重启并用旧 meta 复核原三元组，精确清理 run-owned `/tmp/daily-song-list-pg-api-code-<run>-<attempt>`。
- 新增 workflow data-inert/identity/rollback 合同回归；targeted `1/1`、完整 next-serving `260/260`（58.089s）、relay `13/13`、Python compile、外层与远端 shell syntax、`git diff --check` 均通过。尚待 commit/push/PR/CI/squash merge、执行一次 code-only 部署并取得真实 identity latency/三元组证据；完成后再复核零 writer 与硬资源门禁，只调度唯一 latest-head WDC。交付未完成。
- PR #120 CI `32699994657`（job `97349415370`）success、squash merge main `811cd90e2d76609646179b5f0ab62fc1f34725af`，main Check `32700132860` success。部署前枚举发现 GitHub 遗留 queued WDC `31903306500`：旧 head `9b9c46d6`、创建于 `2026-08-15T19:13:26Z`、8 天未产生任何 job；普通 cancel 与 force-cancel 均由 GitHub API 返回 500。现有 WDC/core release window 会忽略超过 3600 秒的 stale queued lease，新 code-only gate 初版却把所有 queued 都当 active，故正确停止在部署前。最小修复让 code-only gate复用同一 3600 秒规则并输出 `PG_API_CODE_STALE_QUEUED_WDC_IGNORED`；fresh queued/in-progress WDC 仍 fail closed。
### 2026-08-25 10:05 Asia/Taipei — Song source raw-group guard must preserve explicit replacement preimages

- WDC run `32769982552` (head `5f3e5df9d53008d0c8ef7852cf235841d80bfbe9`) completed all views and owner/preflight gates, then failed at affected-parent source `61f3f7a91681f6cf`: ranking `(255,1,255,255)`, source `(257,1,257,257)`. The persisted card is `とても素敵な6月でした::eight`; two overlay-only `六月` rows formed a separate ranking card but PR #128's display-owner fallback merged them into the persisted source. No transport, identity, or resource-cap failure; exact cleanup removed the fixed volume/root/relay and release stayed unchanged.
- Minimal fix in `server/pg_adapter.py` now requires the persisted raw ranking group to agree with the broad display-owner group before ordinary Song candidates can fall back to a persisted source. A punctuation/title variant is admitted only when a same-video replacement explicitly names the persisted `parentSongGroupKey`, preserving unique preimage deletion without merging unranked overlay cards. New regression `test_snapshot_song_source_does_not_merge_unranked_display_alias` locks the 61f3 shape; the existing `Old-Song` unique-preimage test remains covered.
- Verification: the new/related source tests `4/4`, all Song-source tests `10/10`, and the full `tests.test_next_serving_v3` suite `268/268` passed; Python compile and `git diff --check` also passed. No new WDC is scheduled until commit/push/PR/CI/squash merge, zero writers, and the fixed resource gates are re-read.

### 2026-08-30 19:55 Asia/Taipei — Song reset owner-key fix merged; latest-head WDC pending

- PR `#164` (`108ad3ba3ca`), which carries the direct `parentSongSourceDetailKey` through accepted-video reset changes, bounded Song owner windows, and ranking owner mapping while preserving source materialization semantics, has squash-merged as latest main `8ebf6632db25e97d1d7f8dab53107e2a5b60c313`.
- Regression evidence: GitHub PR CI run `33309820833` / job `99252738923` passed (`Test next serving v3`); main Check code run `33309916353` / job `99252988199` passed. WSL full next-serving was `302/302`, relay/two-day `19` with `2` skipped, Python compile and `git diff --check` passed. The production-shaped owner regression asserts `578 -> 579` replacement inclusion; prior failed WDC `33306660304` remains the latest production failure and is not reclassified until a fresh WDC proves it.
- Current delivery is still incomplete: latest known accepted tuple is `accepted_32589432063_1` / content `86fbd44bd80a4e21...` / source `7c7f7c339c40fb0d...`; WDC still serves release `3cfb9f8b327534d8f52ec397f2527d849f879708b105d0857de8e5b92d977d85`. No WDC builder is running locally and no fresh WDC workflow is active; the last failed run cleaned its owner-marked volume, relay root, and build root.
- Next step: after re-reading latest main/accepted/core/WDC identities, zero duplicate writers, stale-lease handling, and every fixed storage/cgroup/relay gate, schedule exactly one latest-head WDC from `8ebf6632db25...`; do not rerun on a failure without its complete checkpoint/exception/resource/cleanup evidence.
