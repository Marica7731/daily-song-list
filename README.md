...
`.github/workflows/sync-wdc-release.yml` uses GitHub-hosted Ubuntu only as a small controller and regression gate. Heavy PostgreSQL snapshot materialization, page generation, serving-store construction, and release assembly run on WDC; neither Windows nor the Mac runner carries database payloads or build databases. The controller transfers a hash-manifested sparse source set below 100 MB, never a repository clone or Git object pack.

VPS2 runs one exact `www-data` relay from loopback TCP to the PostgreSQL Unix socket. WDC opens the strict-known-host SSH tunnel directly to that relay. The relay has a cumulative bidirectional wire ceiling of `16,000,000,000` bytes and at most two concurrent connections. `materialize-pg-release-snapshot.py` reads every page, scope, source detail, and occurrence inside one `REPEATABLE READ READ ONLY` transaction. Only exact transport failures may reconnect; every reconnect rechecks the active revision, content hash, and source commit.

Each attempt owns `/opt/culua/ytb-song-rank/.build/dsl-wdc-<run>-<attempt>`, `/var/tmp/dsl-wdc-volume-<run>-<attempt>`, and `/run/dsl-wdc-<run>-<attempt>`. The owner-marked `/var/tmp` root must share the project filesystem but stays outside the project tree. Its cgroup is fixed at 2.5 GiB RAM, 1 GiB swap, three CPUs, 96 tasks, and nine hours. Hash-locked Linux wheels from `scripts/migration/requirements-wdc-linux.txt` install only into the run volume. The workflow runs once daily rather than every 20 minutes, so a long build does not manufacture duplicate scheduled runs.

There is no release tarball. After the stable release database and exact source invariants pass, page/dependency/frontend intermediates are removed from the fixed volume; the immutable release is copied once to `releases/.incoming-<sha>.<run>-<attempt>` and same-filesystem renamed into place. Failure cleanup stops only exact run units and removes only owner-marked paths. Active or rollback-protected candidates are preserved.
...
- One exact sparse ext4 loop filesystem has a fixed `32,000,000,000`-byte logical capacity under the exact owner-marked `/var/tmp` root. The checker separately enforces project logical bytes and allocated host blocks without following symlinks; keeping the volume outside the project prevents its sparse logical length from inflating project size.
- The final immutable release must remain below `16,000,000,000` logical bytes. Empty, symlinked, non-regular, missing, or ambiguous release content fails closed.
- Before the first large write, prove `host free - 32,000,000,000 >= 20,000,000,000`, `current logical project + 134,217,728 < 40,000,000,000`, and `current logical project + 16,000,000,000 + 134,217,728 < 40,000,000,000`.
- If any project peak reaches `40,000,000,000`, host reserve would fall below `20,000,000,000`, or any input cannot be proved, the deployment **must stop before writing to WDC**.
- A separate guard checks capacity every 30 seconds. The workflow preserves at least `20,000,000,000` bytes of host filesystem headroom at preflight, throughout build/copy, and after finalization.
- WDC writes and cleanup are limited to the exact owner-marked build/secret roots, exact incoming candidate, exact 64-hex release, and installer control targets. Never scan or delete sibling projects or unrelated temporary files to make room.
