...
4. `index.html` + `assets/app.js` render the latest data and allow switching to an hourly snapshot.
   ...
   - Song ranking and song index summaries distinguish current visible videos from the range/catalog video directory. If hiding unknown-artist rows removes every visible song from some videos, the main metric says `可见N视频` and the note reports the current range directory instead of showing two unexplained video totals.
