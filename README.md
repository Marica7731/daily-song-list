...
4. `index.html` + `assets/app.js` render the latest data and allow switching to an hourly snapshot.
   ...
   - Ranking rows preview one primary source channel inline; source buttons open an inline drawer grouped by video, with sorted timestamp links, whole-video setlist copy, and one song-level `复制全部链接` action that copies unique source videos in `频道名 https://www.youtube.com/watch?v=VIDEO_ID&t=SECONDSs` line format.
   ...
   - Compact mobile and tablet song rows keep source actions and trend badges on the second line: repeated count text is shortened to `来源`, differing video counts use text such as `25源`, single-video multi-timestamp rows use `3点`, and one-source rows show a compact copy icon that writes the same `频道名 https://www.youtube.com/watch?v=VIDEO_ID&t=SECONDSs` line format.
   - Source drawers are grouped by video and initially show 3 videos on mobile, 6 on tablet, and 9 on desktop. `查看更多来源` appends all remaining source videos with one user click and internally yields between 12-video chunks, while artist-song drawers append additional songs in batches of 8. Desktop can keep multiple rows expanded, while compact modes replace the previously opened drawer and evict older closed drawer DOM after a small LRU limit.
