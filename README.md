...
4. `index.html` + `assets/app.js` render the latest data and allow switching to an hourly snapshot.
   ...
   - `FrontendUtils.sourcePresentationModel` inlines 0-3 unique source videos. Four or more source videos show the first three inline plus `查看其余 N 个来源`; one click renders all remaining sources and changes the control to `收起其余来源`. One-source rows no longer open drawers.
   ...
   - Song ranking and song index summaries keep the source video count aligned with the active runtime payload. If hiding unknown-artist rows removes every visible song from some videos, the summary note reports the current visible video count separately instead of shrinking the range's source video total.
