...
4. `index.html` + `assets/app.js` render the latest data and allow switching to an hourly snapshot.
   ...
   - Each inline source includes a micro video thumbnail, channel link, timestamp meta link, optional extra-time toggle, and compact setlist-copy icon. Mobile rows keep the collapsed preview to two real inline sources and use a compact `+N来源` / `收起` action for any remaining source videos, including exactly three-source rows. Tablet and desktop proof fixtures lock the wider 3-source inline layout. The expanded source toolbar exposes the song-level copy action.
