...
The runtime workflow uploads the small API, query, activation, and service-file set from the Mac runner before preparing VPS2. The production checkout does not require `/opt/culua/ytb-song-rank/.git`; if that history directory is removed to reclaim disk, the workflow skips the remote Git fetch and verifies the runner-uploaded support files instead. The active database and static `data` tree remain separate and are never removed by this mode.

The publish step first checks the free bytes under `/var/lib/culua/ytb-song-rank`. When the host can hold the active DB plus a run-scoped candidate DB and 1 GiB of margin, it copies the active database to a path such as `song-rank.sqlite.next.<run>.<attempt>`, then transfers the GitHub-built SQLite file there. The active database is not touched until the candidate sha256 and query probe pass inside `song-rank-db-activate.sh`.

If VPS2 does not have enough room for both the active and candidate DB, the workflow fails before stopping the API or touching the active database. The current service and database therefore remain available; add disk capacity or reclaim only verified stale files before retrying. There is deliberately no direct in-place overwrite fallback, because that path can cause a long 502 window and has no `.previous` rollback copy.
