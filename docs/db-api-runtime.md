...
- Default `searchScope=all` for `songs`, `songIndex`, `artists`, and `vsingerSongs` is intentionally identity-focused when `searchFields` is omitted: a query must match visible song/artist/entity fields, not merely a channel name, video title, or parsed source note. The user-facing song search normally sends `searchFields=title,artist` so `歌名` and `歌手` can be removed independently as chips. Removing both fields sends `searchFields=all` for the current request and broadens matching to channel/video/source context; refreshing the page returns to the default `歌名` + `歌手` fields. Use explicit `searchScope=channel`, `video`, or `source` when a diagnostic URL must stay narrowed to one source-context family. All scopes still match complete continuous query terms; partial character reordering and wildcard expansion are not allowed.
...
- Source search acceleration is deliberately narrower than the source detail data. `source_occurrences` remains the complete auditable source table for `/api/sources/{sourceDetailKey}` and copy/jump interactions, while the FTS tables contain one indexed row per unique `(source_key, position)`. The API materializes FTS/LIKE matches into a temporary `matched_sources` table before joining back to `source_details` and `ranking_rows`, so all-field source searches do not repeatedly expand the same source rows across count/video/index ranking variants.
