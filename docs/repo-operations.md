...
静态 workflow 不再要求 VPS2 本地 Git checkout fetch main，而是先读取 `data/ui/meta.json`，再按当前 meta/manifest 稀疏 checkout 有效的分页、来源和搜索分片；它明确排除根级完整 range JSON、legacy 路径和 `data/latest.json`，并从 GitHub runner 将 index、assets、静态 data 文件以 tar 流直接传到既有 web root。写入前先用 `df -Pk` 只读检查远端空间，不足时输出 `CODEX_STATIC_DEPLOY_BLOCKED`/`CODEX_STATIC_RECOVERY_BLOCKED` 并退出，不再半写入首页。meta 在分片落地后最后替换；通过预检后才执行逐文件完整性检查、nginx 检查和 reload。精简范围读取失败时页面直接显示可诊断错误，不再静默切换到大型 JSON。它不替代 runtime DB 的上传空间门禁，runtime DB 仍必须走 `deploy-runtime-db.yml` 的安全上传/激活流程。
