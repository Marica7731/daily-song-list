...
- 平板和桌面内联来源固定保留约 92px 操作轨；1 个、2 个、3+ 来源的卡片宽度必须落在同一预览轨道，不得让单来源卡片横向拉满整行。手机仍使用单列全宽来源卡片。
...
- 普通榜单默认页大小由 `FrontendUtils.responsiveListPageSize` 统一维护：桌面/平板默认 50 条，手机默认 20 条；显式 URL 或查询面板选择的 `pageSize` 继续生效。
- 来源展开抽屉独立分页：桌面/平板每页最多 20 个来源视频，手机每页最多 10 个来源视频。展开时必须先显示加载/错误状态，不得静默空白；API 模式请求 `/api/sources/{sourceDetailKey}?page=...&pageSize=...`，静态详情 shard 则只渲染当前页作为兼容 fallback。
