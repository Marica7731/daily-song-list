...
- 来源展开抽屉独立分页：桌面每页最多 30 个来源视频，手机/平板每页最多 20 个来源视频。展开时必须先显示加载/错误状态，不得静默空白；API 模式请求 `/api/sources/{sourceDetailKey}?page=...&pageSize=...`，静态详情 shard 则只渲染当前页作为兼容 fallback。
