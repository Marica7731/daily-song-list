...
- 频道行必须显示头像。优先使用后端 `avatarUrl` / `channelAvatarUrl` 等频道头像字段；字段缺失或图片加载失败时使用稳定本地 fallback，不得显示破图。
- 已进入曲库或来源目录的频道显示 `已收录` tag。前端兼容 `isCollected`、`knownSourceType` 等字段缺失，缺字段时不报错、不显示空 tag。
