...
- 搜索输入固定在面板顶部，始终可见。
- 面板内部不得再拆成 `搜索` / `筛选` 两个 tab。匹配建议、内容范围、排行条件、显示设置和历史快照必须处于同一个可滚动面板，手机端尤其不能让搜索词和筛选条件分属两个 tab；不要展示最近搜索块。
- 默认进入页面时显示无歌手收录；`隐藏无歌手` 是正向缩小结果集的筛选条件，只有开启后才显示 chip 和计入条件数量。新 URL 只序列化 `hideUnknown=1`，不再写 `showUnknown`；旧 `showUnknown` 仅作为入站兼容。
- 可删除 chip 只能代表真实缩小结果集的条件：搜索词、只看小众、隐藏无歌手、趋势筛选和最低收录。排行口径、每页数量、历史快照和视频布局不得显示为可删除筛选 chip，也不得计入查询按钮数量。`清除全部` 只清除这些缩小条件，必须保留排行口径、每页数量、当前 range/view、历史快照和视频布局。
...
- 平板 3 来源内联 fixture。
- 手机歌曲榜、歌手榜、歌曲索引、索引中页、索引末页、视频页、展开视频顶部和底部收起入口、active query strip、query suggestions、query filter、query history、展开来源。
