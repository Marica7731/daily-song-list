...
- 3 个唯一来源视频：移动端默认内联前 2 个真实来源，剩余 1 个来源通过紧凑 `+1来源` 操作进入展开区；平板和桌面 proof fixture 覆盖 3 个来源布局。复制同一首歌全部来源链接是固定约 28px 图标按钮，不得占据等宽空卡片。
...
- 3+ 折叠态移动端允许 2 个真实内联来源和 1 个紧凑复制全部入口；平板/桌面 3 来源 fixture 使用 `.source-inline-tail`，第三个来源占剩余宽度，`+N来源` / `收起` 按钮自适应文字且最大宽度约 92px。
