...
- VTuber 频道展开的歌曲卡片使用同一套 30/20 分页：桌面每页 30 首，H5/平板每页 20 首。桌面卡片区使用三列密排；H5 使用单列横向信息卡，缩略图、歌名、歌手和次数都靠左/靠右规则明确，不得把标题居中或在窄屏硬塞两列。
...
- 已进入曲库或来源目录的频道显示 `已收录` tag。tag 必须占用独立网格行，不得和频道名争用同一横向空间；移动端频道名、tag、代表曲目应从同一内容列左边界开始对齐。
- 排序支持歌唱次数、曲目数、歌枠/视频数量；默认仍是歌唱次数。`metric=songs` 只用于 VTuber 频道页，切到其它榜单时回到已有排序口径。
- 频道行不得同时重复展示同义曲目指标；内容列只展示代表歌曲预览和视频数，曲目总数放在右侧展开 chip 或当前排序值。
- 曲目展开不得一次性渲染几千首。VTuber 频道初次展开只显示 4 首曲目，后续每次最多追加 12 首；歌手榜保留原来的 8 首初始、8 首追加批量。
