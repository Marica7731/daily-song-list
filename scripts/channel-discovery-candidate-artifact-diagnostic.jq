...
def safe_check($label; $expression):
  {
    label: $label,
    passed: (try ($expression == true) catch false)
  };
...
. as $records |
[
  safe_check("records-array"; $records | type == "array"),
  safe_check("request-contract";
    $request.kind == "channel-discovery-candidate-run" and
    $request.candidateOnly == true and
    ($request.forceRefresh | type == "boolean")),
  safe_check("force-refresh-binding";
    ($source.forceRefresh | type == "boolean") and
    $source.forceRefresh == $request.forceRefresh and
    ($checkpoint.forceRefresh | type == "boolean") and
    $checkpoint.forceRefresh == $request.forceRefresh),
  safe_check("completion-contract";
    $source.schemaVersion == 1 and $source.complete == true and $source.partial == false and
    $checkpoint.schemaVersion == 1 and $checkpoint.complete == true and $checkpoint.partial == false and
    $checkpoint.discoveryCheckpoint.schemaVersion == 1 and
    $checkpoint.discoveryCheckpoint.complete == true and
    $checkpoint.discoveryCheckpoint.partial == false),
  safe_check("request-identity";
    exact_identity($request; $request; $expectedSourceCommit; $expectedChannelId; $expectedHandle; $expectedChannelUrl)),
  safe_check("source-identity";
    exact_identity($source; $request; $expectedSourceCommit; $expectedChannelId; $expectedHandle; $expectedChannelUrl)),
  safe_check("checkpoint-identity";
    exact_identity($checkpoint; $request; $expectedSourceCommit; $expectedChannelId; $expectedHandle; $expectedChannelUrl)),
  safe_check("source-kind";
    $source.kind == "channel-discovery-source-manifest" and
    $source.candidateOnly == true and
    $source.sourceReachedEnd == true),
  safe_check("page-contract"; initial_pages_bound($source.pageSummaries; $request)),
  safe_check("page-evidence-index";
    ($source.pageEvidenceFiles | type == "array") and
    ($source.pageEvidenceFiles | length) == (all_evidence_paths($source.pageSummaries) | length) and
    (all_evidence_paths($source.pageSummaries) | sort) == ([$source.pageEvidenceFiles[] | .path] | sort) and
    all($source.pageEvidenceFiles[];
      (.path | test("^pages/[0-9]{2}-(streams|videos)(-continuation-[0-9]{3}\\.json|\\.html)$")) and
      (.sha256 | test("^[a-f0-9]{64}$")) and
      (.bytes | type == "number" and . >= 0)) and
    all($source.pageSummaries[];
      . as $page |
      any($source.pageEvidenceFiles[];
        .path == $page.evidencePath and .sha256 == $page.rawSha256 and .bytes == $page.bytes) and
      all($page.continuationEvidence[];
        . as $continuation |
        any($source.pageEvidenceFiles[];
          .path == $continuation.evidencePath and
          .sha256 == $continuation.sha256 and
          .bytes == $continuation.bytes)))),
  safe_check("checkpoint-binding";
    ...
    ($checkpoint.discoveryCheckpoint.pageSummaries | type == "array") and
    $checkpoint.discoveryCheckpoint.pageSummaries == $source.pageSummaries and
    initial_pages_bound($checkpoint.discoveryCheckpoint.pageSummaries; $request)),
  safe_check("candidate-count-binding";
    $source.candidateCount == ($records | length) and
    $source.candidateCount > 0 and
    $source.candidateCount <= $request.maxVideos),
  safe_check("record-video-ids";
    ($records | map(.youtubeVideoId | type == "string" and length > 0) | index(false) == null) and
    ($records | map(.youtubeVideoId) | length == (unique | length))),
  safe_check("record-identities";
    $records | map(exact_record_identity(.; $request)) | index(false) == null),
  safe_check("record-evidence";
    $records | map(record_evidence_bound(.; $source.pageSummaries; $request)) | index(false) == null)
] as $checks |
{
  schemaVersion: 1,
  failedChecks: [$checks[] | select(.passed != true) | .label],
  counts: {
    recordCount: (try ($records | length) catch null),
    sourceCandidateCount: ($source.candidateCount | safe_number),
    checkpointCandidateCount: ($checkpoint.candidateCount | safe_number),
    discoveryCheckpointCandidateCount: ($checkpoint.discoveryCheckpoint.candidateCount | safe_number),
    pageSummaryCount: (try ($source.pageSummaries | length) catch null),
    pageEvidenceFileCount: (try ($source.pageEvidenceFiles | length) catch null),
    continuationRoundCount: (try (([$source.pageSummaries[] | .continuationRounds | select(type == "number")] | add) // 0) catch null),
    inheritedInitialOwnerCount: (try (([$source.pageSummaries[] | (.inheritedInitialOwnerVideoIds // []) | length] | add) // 0) catch null),
    inheritedContinuationOwnerCount: (try (([$source.pageSummaries[].continuationEvidence[]? | (.inheritedOwnerVideoIds // []) | length] | add) // 0) catch null),
    invalidRecordIdentityCount: (try ([$records[] | select((exact_record_identity(.; $request)) != true)] | length) catch null),
    invalidRecordEvidenceCount: (try ([$records[] | select((record_evidence_bound(.; $source.pageSummaries; $request)) != true)] | length) catch null)
  }
}
