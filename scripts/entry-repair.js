...
function cleanSafeTitleCandidate(value) {
  ...
  text = text
    ...
    .replace(
      /^(?:未記載|未记载|待补歌手|待補歌手|待补|待補)\s+(?=(?:[⟦［\[]\s*#?[\d０-９]{1,3}\s*[⟧］\]]|[\u2460-\u2473\u3251-\u325f\u32b1-\u32bf]|[mｍ]?\d{1,3}[.．]|[#＃]?\d{1,3}\s*[≫»>]|[#＃]?\d{1,3}\s*[)）、:：]|[#＃]?[\d０-９]{1,3}\s+[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}]))/iu,
      "",
    )
    .trim();
  ...
  text = text
    .replace(
      /^(?:[⟦［\[]\s*#?[\d０-９]{1,3}\s*[⟧］\]]\s*|[\u2460-\u2473\u3251-\u325f\u32b1-\u32bf]\s*|[mｍ][\d０-９]{1,3}[.．]\s*|[\d０-９]{1,3}\s*[≫»>]+\s*|[#＃]?[\d０-９]{1,3}[.．](?![\d０-９])\s*|[#＃]?[\d０-９]{1,3}\s*[)）、:：]\s*|[#＃]?[\d０-９]{1,3}\s+(?=[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}]))/iu,
      "",
    )
    .trim();
