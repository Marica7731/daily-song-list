...
function channelDiscoveryOptionsFromArgs(args, defaults = {}) {
  ...
  return {
    ...
    inspectMaxAttempts: positiveInteger(args["inspect-max-attempts"], defaults.inspectMaxAttempts ?? 3, "--inspect-max-attempts"),
    requestIntervalMs,
...
async function runChannelDiscovery(options, deps) {
  ...
  const manifest = {
    ...
    inspectMaxAttempts: options.inspectMaxAttempts,
