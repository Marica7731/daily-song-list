...
test("high-density rank and source rules are encoded in css and browser checks", () => {
  ...
  assert.match(indexSource, /id="querySearchPanel"[\s\S]*id="queryFilterPanel"/u);
  assert.doesNotMatch(indexSource, /id="queryFilterPanel"[^>]*hidden/u);
  ...
  assert.match(cssSource, /\.query-panel-body\s*\{[\s\S]*display: grid;[\s\S]*gap: var\(--space-4\);/u);
  assert.doesNotMatch(cssSource, /\.query-tabs\b|\.query-tab\s*\{|\.query-tab\[|\.query-panel\.is-filter-tab/u);
  ...
  assert.match(appSource, /if \(!els\.queryTabButtons\.length\)/u);
