...
test("VTuber channel expansion renders paged song groups before source pages", () => {
  ...
  assert.match(functionBody("function artistLabelForSongGroup"), /if \(displayArtist && !window\.RankingUtils\.isUnknownArtistName\(displayArtist\)\) return displayArtist;/u);
  ...
  assert.match(functionBody("function artistLabelForSongGroup"), /\(knownEntries\.length \? knownEntries : entries\)/u);
