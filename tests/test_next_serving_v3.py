...
class Tests(unittest.TestCase):
    ...
    def test_source_preimage_legacy_parent_matches_canonical_same_video_replacement(self):
        record={"video":{"videoId":"video-one"},"occurrences":({
            "seconds":2667,"title":"逆光","artist":"Ado",
        },)}
        change={
            "entityType":"runtime_occurrences","videoId":"video-one",
            "occurrenceId":"occ-canonical","rangeId":"all",
            "position":99,"seconds":2667,
            "title":"逆光(ウタ from ONE PIECE FILM RED)","artist":"Ado",
            "replacement":True,"replacementSameVideo":True,
            "_parentRuntimeOccurrenceExists":True,
            "_runtimeOccurrenceOwnerWasExplicit":False,
            "replacementPayload":{
                "videoId":"video-one","occurrenceId":"occ-canonical",
                "rangeId":"all","seconds":2667,
                "title":"逆光","artist":"Ado",
            },
        }
        self.assertTrue(
        ...
        wrong_replacement=dict(
            ...
            replacementPayload=dict(
                change["replacementPayload"], title="別の曲",
            ),
        )
        self.assertFalse(
            pg_adapter._source_record_matches_change(record,wrong_replacement),
        )

    def test_snapshot_source_legacy_parent_replacement_is_replaced_once(self):
        parent={
        ...
        change={
            "entityType":"runtime_occurrences","videoId":"video-one",
            "occurrenceId":"occ-canonical","rangeId":"all",
            "position":99,"seconds":2667,"title":"song (raw)",
            "artist":"Ado","replacement":True,
            "replacementSameVideo":True,
            "_parentRuntimeOccurrenceExists":True,
            "_runtimeOccurrenceOwnerWasExplicit":False,
            "replacementPayload":{
    ...
    def test_snapshot_song_source_accepts_canonical_reset_owner_without_preimage(self):
        source_key = "source-canonical-reset-owner"
        owner_key = "メロメラ::ilife"
        parent = ({
            "videoId": "parent-video", "occurrenceId": "parent-occurrence",
            "rangeId": "all", "position": 0, "seconds": 101,
            "title": "メロメラ", "artist": "iLiFE", "songKey": owner_key,
        },)
        candidate = {
            "revision_id": "accepted-boundary",
            "video_id": "reset-video",
            "occurrence_id": "reset-video:0:202", "position": 0,
            "range_id": "all", "song_key": "raw-reset-key", "seconds": 202,
            "title": "メロメラ", "artist": "iLiFE!",
            "source_id": "fixture-source", "source_system": "core-7d",
            "video_title": "Reset video", "channel_id": "UCfixture",
            "channel_name": "Fixture", "occurrence_payload_json": {},
            "video_payload_json": {}, "video_tombstone": False,
        }
        payload = pg_adapter._snapshot_materialized_source_payload(
            ...
            range_id="all",
            persisted_record={
                "type": "song", "key": owner_key, "title": "メロメラ",
                "sourceDetailKey": source_key, "rangeId": "all",
            },
            targets=(("songs", owner_key),),
            ...
            accepted_video_resets={"reset-video": {
                "video_id": "reset-video", "payload_json": {"rangeId": "all"},
            }},
            runtime_changes=(),
        )
        self.assertTrue(payload["found"])
        ...
        self.assertIn(
            "reset-video", {item["videoId"] for item in record["occurrences"]},
        )

    def test_snapshot_song_source_rejects_unannotated_reset_with_multiple_targets(self):
        """A shared reset scope must not broaden an unannotated Song row."""
        source_key = "source-ambiguous-reset-scope"
        owner_key = "メロメラ::ilife"
        parent = ({
            "videoId": "parent-video", "occurrenceId": "parent-occurrence",
            "rangeId": "all", "position": 0, "seconds": 101,
    ...
    def test_reconcile_authoritative_boundary_payload_proves_mixed_marker_subset(self):
        initial = {
            "found": True,
            "record": {
        ...
        unrelated = tuple(
            {
                "_authoritative_7d_overlay": True,
                "video_id": f"unrelated-{index}",
                "occurrence_id": f"unrelated-{index}-0",
            }
            for index in range(3)
        )
        candidate_rows = (ordinary, redundant, legitimate) + unrelated
        calls = []
        ...
        def build(rows):
            ...
            if has_legitimate and not has_redundant:
                count, videos = 411, 408
            elif has_redundant and not has_legitimate:
                count, videos = 412, 407
            elif has_redundant and has_legitimate:
        ...
        reconciled = pg_materializer._reconcile_authoritative_boundary_payload(
            source_key="source-mixed-marker-subset",
            payload=initial,
            candidate_rows=candidate_rows,
    ...
    def test_reconcile_authoritative_boundary_payload_proves_runtime_ordinary_pair(self):
        initial = {
            "found": True,
            "record": {
                "count": 413, "songCount": 1, "videoCount": 409,
        ...
        def build(rows, changes):
            ...
            has_ordinary = ordinary_duplicate in changes
            has_redundant = redundant in rows
            has_legitimate = legitimate in rows
            if has_ordinary:
                if has_redundant and has_legitimate:
        ...
        reconciled = pg_materializer._reconcile_authoritative_boundary_payload(
            source_key="source-runtime-ordinary",
            payload=initial,
            candidate_rows=(redundant, legitimate),
            expected_cardinality=(411, 1, 408, 411),
            build_payload=lambda rows: build(rows, (ordinary_duplicate,)),
            ordinary_rows=(ordinary_duplicate,),
    ...
    def test_reconcile_authoritative_boundary_payload_proves_mixed_runtime_pair_within_global_budget(self):
        initial = {
            "found": True,
            "record": {
                "count": 413, "songCount": 1, "videoCount": 409,
        ...
        runtime_padding = tuple(
            {
                "video_id": f"runtime-padding-{index}",
                "occurrence_id": f"runtime-padding-{index}-0",
            }
        ...
        def build(rows, changes):
            ...
            is_target = (
                ...
                and ordinary_candidate not in rows
                and runtime_ordinary not in changes
                and all(item in changes for item in runtime_padding)
            )
            if is_target:
                count, videos = 411, 408
            else:
                # Padding rows keep the runtime pair search just above the
                # legacy standalone cap.  Removing only the required runtime
                # row is authoritative; removing any padding row remains a
                # distinct, non-authoritative result so uniqueness is proved.
                if redundant in rows or legitimate in rows:
                    count, videos = 413, 407
                elif ordinary_candidate not in rows:
                    count, videos = 410, 407
        ...
        runtime_rows = (runtime_ordinary, *runtime_padding)
        reconciled = pg_materializer._reconcile_authoritative_boundary_payload(
            source_key="source-mixed-runtime-pair-budget",
            payload=initial,
            candidate_rows=(ordinary_candidate, redundant, legitimate),
            expected_cardinality=(411, 1, 408, 411),
            build_payload=lambda rows: build(rows, runtime_rows),
            ordinary_rows=runtime_rows,
            build_payload_with_ordinary=build,
        )
        self.assertEqual(
            pg_materializer._source_payload_cardinality(reconciled["record"]),
